...
function ShortcutRow({ label, hint }: ShortcutRowProps) {
  return (
    <div className="flex items-center gap-1 px-2 py-[6px] rounded-[4px] w-full">
      <span className="flex-1 text-[14px] leading-[20px] text-[#18181b]" style={{ fontFamily: "'DM Mono', monospace", fontWeight: 400 }}>
        {label}
      </span>
      <span className="text-[12px] leading-[16px] text-[#71717a] opacity-60 shrink-0" style={{ fontFamily: "'DM Mono', monospace", fontWeight: 400 }}>
        {hint}
      </span>
    </div>
  )
  ...
  return (
    <div className="flex flex-col items-start px-1 py-0 w-full shrink-0">
      <div className="flex items-center gap-2 px-2 py-[6px] w-full">
        <span className="text-[12px] leading-[16px] text-[#a1a1aa]" style={{ fontFamily: "'DM Mono', monospace", fontWeight: 500 }}>
          {title}
        </span>
      </div>
      {rows.map((r) => (
...
function Separator() {
  return <div className="w-full shrink-0 h-px bg-[#e4e4e7]" />
}

export function ShortcutsPanel() {
  return (
    <div
      className="flex flex-col gap-2 py-1 rounded-lg border border-[#e4e4e7] bg-white"
      style={{
        width: 347,
